...
- Platform integration: [`platforms/network`](../../platforms/network/) ships both `install` and `bundle` per registered cluster (default-enabled, opt-out via `network-platform/trust-manager-{install,bundle}: "false"`) — the bundle ApplicationSet ships a fixed `cluster-trust-bundle` combining default CAs + `cluster-ca-secret` into ConfigMap `cluster-trust-bundle` (key `trust-bundle.pem`).
