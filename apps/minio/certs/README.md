...
Internal sub-chart used by [`apps/minio/install`](../install/). Renders a list of cert-manager `Certificate` resources from a single list-shaped values input. Not intended for direct consumption — the parent `minio` app-of-apps Application drives its values via `helm.valuesObject`.
