...
<details>
<summary><b><code>apps/</code> — user-facing applications</b> (6 of them listed)</summary>
...
| [`homerun2`](./apps/homerun2/) | `install` (+ `httproute` / `secrets` / `smoke-test` / `preview-*` sub-charts) | 13 components, each pinned separately — see its README | App-of-apps for the homerun2 event bus: redis-stack + omni/demo/git/k8s pitchers, core/led/light/notification catchers, scout, config-viewer, wled-mock |
...
| [`tabletennis`](./apps/tabletennis/) | `install` | delegates to `schmetterpause` + `zaehlwerk` | **The tabletennis platform**, beside homerun2 rather than inside it — one Application rendering schmetterpause + zaehlwerk, plus the optional LED strip at the table. Talks to homerun2 over the event bus and knows only the namespace it runs in |
...
<details>
<summary><b><code>platforms/</code> — pre-bundled ApplicationSets per cluster role</b> (7 bundles)</summary>
...
| [`platforms/homerun2`](./platforms/homerun2/) | `homerun2-platform: "true"` + `clusterbook…/allocation-ip` non-empty + gate label `homerun2-platform.stuttgart-things.com/secrets-config` | 1 appset — the homerun2 event bus (redis-stack, omni-pitcher, core-catcher, scout, led-catcher), their routes and ExternalSecrets | App-of-apps parent, so `cascadingDelete: false` and no `preserveResourcesOnDeletion` (#324): an opt-out stops Argo managing the stack rather than deleting it. The component set is fixed — an AppSet can only template strings, so a `<component>.enabled` boolean cannot come from a label |
| [`platforms/tabletennis`](./platforms/tabletennis/) | `tabletennis-platform: "true"` + `clusterbook…/allocation-ip` non-empty + gate label `tabletennis-platform.stuttgart-things.com/secrets-config` | 1 appset — schmetterpause + its CNPG database, zaehlwerk, and (opt-in via `tabletennis-platform/light-catcher`) the LED strip at the table | A platform beside homerun2, not inside it: it knows one thing about it, the namespace. **Requires `storage-platform/cloudnative-pg: 'true'`.** The database survives an opt-out — the chart's own `database.protect` keeps Prune/Delete off the CNPG Cluster |
